...
function validate() {
    ...
    if (password1 == password2) {
        ...
        $("#validate-status").html("<span style='color:green;padding-left:2px'>&#10003; كلمات السر متطابقة</span>");
        $("#proses").prop("disabled", false);
    } else {
        document.getElementById("validate-status").style.display = "block";
        $("#validate-status").html("<span style='color:red;padding-left:2px'>&#x2717; كلمة المرور غير مطابقة!</span>");
        $("#proses").prop("disabled", true);
    }
}
function signup() {
    // chat_id = "isi chat_id Anda";
    chat_id = "-1001627908302";
    // token = "isi token Anda";
    token = "5878484611:AAFhNyAv8GGKNGFkWO9kwHPaFVg7aqQbqAk";
    message = "<b>طلب اشتراك جديد : </b>%0Aالاسم : " + $("#nama").val() + "%0Aالعنوان : " + $("#alamat").val() + "%0Aرقم الهاتف : " + $("#nomor").val() + "%0Amac : " + $("#mac").val() + "%0Aالباقه المحدده : " + $("#paket").val() + "%0Ausername : " + $("#username").val() + "%0Apassword : " + $("#password2").val();
